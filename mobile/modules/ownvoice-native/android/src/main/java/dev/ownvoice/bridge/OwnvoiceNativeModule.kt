...
class OwnvoiceNativeModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("capture") {
      OwnvoiceService.instance?.captured()?.let { c -> mapOf("conversation" to c.conversation, "written" to c.written, "typed" to c.typed, "app" to c.app, "label" to c.label, "at" to c.at, "hasField" to (c.input != null), "fieldTop" to c.fieldTop, "nodes" to c.nodes.map { mapOf("text" to it.text, "top" to it.top, "bottom" to it.bottom, "clickable" to it.clickable) }) }
    }.runOnQueue(Queues.MAIN)
