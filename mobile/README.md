...
The Home screen has a fictional chat with Sam and an empty **Message** field for practice. Turn on Ownvoice in Android accessibility settings from the app, then enable the app you want under **Where the bubble shows** (Ownvoice itself starts off, so switch it on to practice there). The bubble appears only in enabled apps while not paused or showing the panel. Tap it to get replies when the field is empty, or improved versions of text already typed; then choose **Insert** or **Copy**. Each card contains one distinct message (up to three); if another attempt fails, any usable drafts already made remain available. The field's **Message** hint is not treated as typed text or captured screen text. If the phone needs to prepare its writing model, the panel shows **Getting Ownvoice ready…** (with a percentage when available) before switching to **Writing…**; unsupported phones and other failures show a plain-language message. Insertion checks the field afterward and copies the draft if it fails. **Recent activity** adds newly collected tap counts when pressed; it is not a saved history. **Clear last screen** forgets the most recent capture.
