...
The ported core checks are in `src/core/__tests__/`; the phone writer has checks in `src/panel/__tests__/`. The ChatGPT response and switch helpers have tests in `src/chatgpt/__tests__/` and `src/core/__tests__`; the draft-quality rules (duplicate filtering, kept layout, dash rule, reply prompts) in `src/core/__tests__/DraftsTest.test.ts`. Slice 5 emulator evidence: [`reports/OWNVOICE-RN-05.md`](reports/OWNVOICE-RN-05.md). Run the commands above for the current suite.
