...
export async function withPhoneFallback(primary: Writer, phone: PhoneWriter, request: DraftRequest): Promise<Choice> {
  try {
    ...
    if (!drafts.length) throw new Error('empty');
    return { drafts };
  } catch {
...
export function chatgptWriter(complete: (prompt: string) => Promise<string[]>): Writer {
  return { write: ({ conversation, written, guide }) => complete(Judge.rewritePrompt(written || '', conversation, guide)) };
}
