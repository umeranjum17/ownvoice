...
- **Every read is logged where you can see it.** **What was read** on the main screen, or in the drafts panel, lists each tap: the app, the time, how many characters it read and the first line, never the full text. The list stays on the phone and each entry is deleted after 30 days. **Wipe everything** clears the list and whatever the last tap read.
